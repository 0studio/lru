...
func TestKeyStringInitialState(t *testing.T) {
	cache := NewLRUCacheKeyString(5)
	l, sz, c := cache.Stats()
	if l != 0 {
...
func TestKeyStringSetInsertsValue(t *testing.T) {
...
func TestKeyStringSetIfAbsent(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	data := &CacheValue{0}
	var k key.String = "1"
...
func TestKeyStringGetValueWithMultipleTypes(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	data := &CacheValue{0}
	var k key.String = "1"
	cache.Set(k, data)
...
func TestKeyStringDelete(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	value := &CacheValue{1}
	var k key.String = "1"
...
func TestKeyStringClear(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	value := &CacheValue{1}
	var k key.String = "1"
...
type PurgeCacheValueKeyString struct {
}

func (cv *PurgeCacheValueKeyString) OnPurge(why PurgeReason) {
	purgeReasonFlag4TestKeyString = why
}

var purgeReasonFlag4TestKeyString PurgeReason

func TestKeyStringDeleteOnPurge(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	value := &PurgeCacheValueKeyString{}
	purgeReasonFlag4TestKeyString = PURGE_REASON_CACHEFULL // init
	var k key.String = "1"

	cache.Set(k, value)
	cache.Delete(k)
	if purgeReasonFlag4TestKeyString != PURGE_REASON_DELETE {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_DELETE, purgeReasonFlag4TestKeyString)
	}

}

func TestKeyStringUpdateOnPurge(t *testing.T) {
	cache := NewLRUCacheKeyString(100)
	value := &PurgeCacheValueKeyString{}
	purgeReasonFlag4TestKeyString = PURGE_REASON_CACHEFULL // init
	var k key.String = "1"
	...
	if purgeReasonFlag4TestKeyString != PURGE_REASON_UPDATE {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_UPDATE, purgeReasonFlag4TestKeyString)
	}
