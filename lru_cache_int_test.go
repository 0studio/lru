...
func TestIntInitialState(t *testing.T) {
	cache := NewLRUCacheInt(5)
	l, sz, c := cache.Stats()
	if l != 0 {
...
func TestIntSetInsertsValue(t *testing.T) {
	cache := NewLRUCacheInt(100)
	data := &CacheValue{0}
	var k int = 1
	cache.Set(k, data)
...
func TestIntSetIfAbsent(t *testing.T) {
	cache := NewLRUCacheInt(100)
	data := &CacheValue{0}
	var k int = 1
...
func TestIntGetValueWithMultipleTypes(t *testing.T) {
	cache := NewLRUCacheInt(100)
	data := &CacheValue{0}
	var k int = 1
	cache.Set(k, data)
...
func TestIntSetUpdatesSize(t *testing.T) {
	cache := NewLRUCacheInt(100)
...
func TestIntSetWithOldKeyUpdatesValue(t *testing.T) {
	cache := NewLRUCacheInt(100)
	emptyValue := &CacheValue{0}
	k := int(1)
	cache.Set(k, emptyValue)
...
func TestIntSetWithOldKeyUpdatesSize(t *testing.T) {
	cache := NewLRUCacheInt(100)
	emptyValue := &CacheValue{0}
	k := int(1)
	cache.Set(k, emptyValue)
...
func TestIntGetNonExistent(t *testing.T) {
	cache := NewLRUCacheInt(100)
...
func TestIntCapacityIsObeyed(t *testing.T) {
	size := int64(3)
	cache := NewLRUCacheInt(100)
...
func TestIntLRUIsEvicted(t *testing.T) {
	size := int64(3)
	cache := NewLRUCacheInt(size)
...
type PurgeCacheValueInt struct {
}

func (cv *PurgeCacheValueInt) OnPurge(why PurgeReason) {
	purgeReasonFlag4TestInt = why
}

var purgeReasonFlag4TestInt PurgeReason

func TestIntDeleteOnPurge(t *testing.T) {
	cache := NewLRUCacheInt(100)
	value := &PurgeCacheValueInt{}
	purgeReasonFlag4TestInt = PURGE_REASON_CACHEFULL // init
	var k int = 1

	cache.Set(k, value)
	cache.Delete(k)
	if purgeReasonFlag4TestInt != PURGE_REASON_DELETE {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_DELETE, purgeReasonFlag4TestInt)
	}

}

func TestIntUpdateOnPurge(t *testing.T) {
	cache := NewLRUCacheInt(100)
	value := &PurgeCacheValueInt{}
	purgeReasonFlag4TestInt = PURGE_REASON_CACHEFULL // init
	var k int = 1
	...
	if purgeReasonFlag4TestInt != PURGE_REASON_UPDATE {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_UPDATE, purgeReasonFlag4TestInt)
	}

}

func TestIntCacheFullOnPurge(t *testing.T) {
	cache := NewLRUCacheInt(1)
	value := &PurgeCacheValueInt{}
	k1 := int(1)
	k2 := int(2)

	cache.Set(k1, value)                          // after this cache is full
	purgeReasonFlag4TestInt = PURGE_REASON_DELETE // init
	cache.Set(k2, value)                          // after this k1 is delete and reason set to  PURGE_REASON_CACHEFULL
	if purgeReasonFlag4TestInt != PURGE_REASON_CACHEFULL {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_CACHEFULL, purgeReasonFlag4TestInt)
	}

}

func TestIntClearOnPurge(t *testing.T) {
	cache := NewLRUCacheInt(1)
	value := &PurgeCacheValueInt{}
	k1 := int(1)
	purgeReasonFlag4TestInt = PURGE_REASON_DELETE // init
	cache.Set(k1, value)                          // after this cache is full
	cache.Clear()
	if purgeReasonFlag4TestInt != PURGE_REASON_CLEAR_ALL {
		t.Errorf("after cache.Delete ,purgeReason should be %d ,but get %d", PURGE_REASON_CLEAR_ALL, purgeReasonFlag4TestInt)
	}

}

func TestIntOnMiss(t *testing.T) {
	fun := func(k int) (Cacheable, bool) {
		return 1, true
	}
	cache := NewLRUCacheInt(1)
	cache.OnMiss(fun)
